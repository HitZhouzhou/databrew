...
# 定义 MULTIPLES 列表
MULTIPLES=(400 800)  # 修改这个列表，加入你希望测试的倍数
...
for MULTIPLE in "${MULTIPLES[@]}"; do
    ...
    #     echo "正在进行模型微调，尝试次数：$((RETRY_COUNT + 1))"
    #     python finetune.py \
    ...
    #     nvidia-smi >> ../../experiments/finetune/finetune_${MULTIPLE}.log
    #     free -h >> ../../experiments/finetune/finetune_${MULTIPLE}.log
    ...
    #         echo "模型微调失败超过最大重试次数，请查看 ../../experiments/finetune/finetune_${MULTIPLE}_$(date "+%Y%m%d_%H%M%S").log 获取详细信息。"
    #         exit 1
    #     fi
    # done
    # STEP2_END=$(date +%s)
